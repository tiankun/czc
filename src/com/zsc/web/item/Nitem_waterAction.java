...
public class Nitem_waterAction extends BaseAction {
	...
	public String show(HttpServletRequest request) throws Exception {	
		...
		//获取各上传文件
		Collection keyan=dataBaseControl.getOutResultSet("select * from t_nitem_file where yue_id=? and item_type=1 and pic_type='可研'", new Object[]{id});
		request.setAttribute("keyan", keyan);
		Collection chushe=dataBaseControl.getOutResultSet("select * from t_nitem_file where yue_id=? and item_type=1 and pic_type='初设'", new Object[]{id});
		request.setAttribute("chushe", chushe);
		Collection shishi=dataBaseControl.getOutResultSet("select * from t_nitem_file where yue_id=? and item_type=1 and pic_type='实施'", new Object[]{id});
		request.setAttribute("shishi", shishi);
		Collection shigong=dataBaseControl.getOutResultSet("select * from t_nitem_file where yue_id=? and item_type=1 and pic_type='施工'", new Object[]{id});
