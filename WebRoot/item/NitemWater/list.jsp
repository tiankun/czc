...
<head>
 ...
 function mySubmit(){
	 ...
	 document.form1.action="${pageContext.request.contextPath}/item/Nitem_water!list.do?biao_shi="+biao_shi+"";
 ...
 function del(id){
	 var biao_shi = parent.document.getElementById("biao_shi").value;
	 if(confirm('确定删除吗？')){
	   window.location="${pageContext.request.contextPath}/item/Nitem_water!delete.do?id="+id+"&biao_shi="+biao_shi+"";
	 }
 }
 </script>
...
<body>
<form action="${pageContext.request.contextPath}/item/Nitem_water!list.do" method="post" name="form1">
...
<c:if test="${fn:length(fn:split(user.biao_shi,'_'))<=3}">
	<input type="submit" value=" 新 增 " onclick="document.form1.action='${pageContext.request.contextPath}/item/Nitem_water!createAdmin.do'"/>
</c:if>
</form>

<ec:table items='list' var="item" view="compact" rowsDisplayed="20" filterable="false"
	action="${pageContext.request.contextPath}/item/Nitem_water!list.do" autoIncludeParameters="true" imagePath="${pageContext.request.contextPath}/images/table/*.gif">
...
<c:if test="${fn:length(fn:split(user.biao_shi,'_'))==1}">
		...
		<ec:column property="操作" title="操作" sortable="false" filterable="false" width="70px">
			<a href="${pageContext.request.contextPath}/item/Nitem_water!show.do?id=${item.id}&">查看</a>&nbsp;&nbsp;
			<c:if test="${item.guan_li==user.guan_li}">
			<a href="${pageContext.request.contextPath}/item/Nitem_water!edit.do?id=${item.id}&">修改</a>&nbsp;&nbsp;
			<a href="#" onClick="del('${item.id}');">删除</a>&nbsp;&nbsp;
			</c:if>
			<a href="${pageContext.request.contextPath}/item/Nitem_yue!uploadList.do?type=1&item_id=${item.id}">月报</a>
		</ec:column>
	</ec:row>
</ec:table>
