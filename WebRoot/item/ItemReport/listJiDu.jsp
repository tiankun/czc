...
<A href="${pageContext.request.contextPath}/item/Item_report!create.do?item=${item}&type=2"><STRONG>====添加进度季报====</STRONG></A>
</c:if>
<TABLE border=1 width="100%">
  <TR class=tHead>
    <TD width="8%" align=center>上报季度</TD>
...
<c:forEach items="${list}" var="item">  
  <TR onMouseOver="this.style.backgroundColor='#fdecae'" onmouseout="this.style.backgroundColor=''">
    ...
    <c:if test="${checkArea==true&&empty item.pz_zhuangtai}">
       &nbsp;|&nbsp;<A href="${pageContext.request.contextPath}/item/Item_report!edit.do?id=${item.id}">修改</A>&nbsp;|&nbsp;<A onclick="return window.confirm('是否确认要删除这条记录？');" href="${pageContext.request.contextPath}/item/Item_report!delete.do?id=${item.id}&item=${item.item}&type=2">删除</A>
    </c:if>
    </TD>
</TR>
</c:forEach>
